# ⛏️ Mining Manager — Complete Setup Guide
...
│                                                             │
│  ┌─────────────────┐     ┌───────────────────────────────┐  │
│  │  Mining Manager │     │  Supporting Services          │  │
│  │  (Streamlit)    │     │                               │  │
│  │  Port: 8503     │────▶│  STAC API    → port 8082      │  │
│  │                 │     │  TiTiler     → port 8008      │  │
...
```bash
git clone git@github.com:Harish-lvrk/mining_manager.git
cd mining_manager
```
...
| Mining Manager | `streamlit run app.py --server.address 0.0.0.0 --server.port 8503`         | 8503 |
...
├── requirements.txt
├── backend/
│   ├── stac_api.py     ← STAC API helpers
│   └── stac_builder.py ← STAC item builder
├── frontend/
│   └── tab_mining.py   ← Mining Manager UI
└── logs/
```
...
| Tiles show wrong colours        | LAN_IP mismatch — fix `config.py`             |
